...
class BioConnect:
	...
	def getAuthenticatorStatus(self):
		...
		if result == False:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unable to get activation status")
		...
		try:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unexpected reply for activation status")
	
		status = reply.get("status","")

		enrolled = 0

		modals = ["face_status", "voice_status", "fingerprint_status", "eye_status"]

		for m in modals:
			if (reply.get(m, "") == "enrolled"):
				enrolled += 1				

		if (status == "active" and enrolled >= 1):
			return "active"
		else:
			return ""
	...
	def sendStepup(self,
		...
		if result == False:
			...
			print(json.dumps(data))
			print(result.content)
			sys.exit("Error: unable to send activation request")
		...
		try:
			...
			print(json.dumps(data))
			print(result.content)
			sys.exit("Error: unexpected reply for activation request")
	...
	def getStepupStatus(self):
		...
		return user_verification.get("status", "")
